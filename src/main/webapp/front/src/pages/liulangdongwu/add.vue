...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="100px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="动物名称" prop="dongwumingcheng">
            <el-input v-model="ruleForm.dongwumingcheng" 
                placeholder="动物名称" clearable :disabled=" false  ||ro.dongwumingcheng"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="动物种类" prop="dongwuzhonglei">
            <el-select v-model="ruleForm.dongwuzhonglei" placeholder="请选择动物种类" :disabled=" false  ||ro.dongwuzhonglei" >
              <el-option
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}'  label="注射疫苗" prop="zhusheyimiao">
            <el-select v-model="ruleForm.zhusheyimiao" placeholder="请选择注射疫苗" :disabled=" false  ||ro.zhusheyimiao" >
              <el-option
                  v-for="(item,index) in zhusheyimiaoOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="体型" prop="tixing">
            <el-input v-model="ruleForm.tixing" 
                placeholder="体型" clearable :disabled=" false  ||ro.tixing"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="动物详情" prop="dongwuxiangqing">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='zhusheyimiao'){
              this.ruleForm.zhusheyimiao = obj[o];
              this.ro.zhusheyimiao = true;
              continue;
            }
            if(o=='tixing'){
              this.ruleForm.tixing = obj[o];
              this.ro.tixing = true;
              continue;
            }
            if(o=='faburiqi'){
        ...
        this.zhusheyimiaoOptions = "已注射,未注射".split(',')
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`liulangdongwu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('liulangdongwu/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`liulangdongwu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
