...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"0","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto","order":"1"}'>
      ...
      <el-form-item :style='{"margin":"0 10px 20px"}'>
        ...
        <el-input v-model="formSearch.zhandiandizhi" placeholder="站点地址" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px 20px"}'>
	  ...
	  <el-button v-if="btnAuth('jiuzhuzhan','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 25px","margin":"0 10px 20px 0","outline":"none","color":"#fff","borderRadius":"0","background":"#57A7A5","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/jiuzhuzhanAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"margin":"0 0 10px","background":"none","order":"4"}'>
		...
		<div :style='{"padding":"0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between","height":"auto"}' class="list list4 index-pv1">
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" :style='{"margin":"20px 0 0","overflow":"hidden","background":"#fff","width":"32%","minWidth":"320px","position":"relative","height":"280px"}' class="item">
			<img @click.stop="imgPreView(item.zhandianfengmian)" v-if="item.zhandianfengmian && item.zhandianfengmian.substr(0,4)=='http'" :src="item.zhandianfengmian.split(',')[0]" class="image" />
			<img @click.stop="imgPreView(baseUrl + (item.zhandianfengmian?item.zhandianfengmian.split(',')[0]:''))" v-else :src="baseUrl + (item.zhandianfengmian?item.zhandianfengmian.split(',')[0]:'')" class="image" />
			<div class="content">
  ...
  export default {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        this.$http.get(`jiuzhuzhan/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/jiuzhuzhanDetail', query: params});
      },
