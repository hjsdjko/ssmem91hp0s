...
<div :style='{"width":"80%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="领养地址" prop="lingyangdizhi">
            <el-input v-model="ruleForm.lingyangdizhi" 
                placeholder="领养地址" clearable :disabled=" false  ||ro.lingyangdizhi"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' label="领养条件" prop="lingyangtiaojian">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="领养条件"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`dongwuzhaojia/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('dongwuzhaojia/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`dongwuzhaojia/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
