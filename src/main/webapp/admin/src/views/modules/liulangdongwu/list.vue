<template>
	<div class="main-content" :style='{"padding":"20px 30px","background":"#f9fbfd","flexDirection":"column","display":"flex"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"flexWrap":"wrap","margin":"20px 0","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('liulangdongwu','动物种类')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(106, 171, 156)","height":"40px"}'></span>
						动物种类
					</el-button>
					<el-button class="btn18" v-if="isAuth('liulangdongwu','动物数量')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(106, 171, 156)","height":"40px"}'></span>
						动物数量
					</el-button>
					<el-button class="btn18" v-if="isAuth('liulangdongwu','健康状态')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(106, 171, 156)","height":"40px"}'></span>
						健康状态
					</el-button>
					<el-button class="btn18" v-if="isAuth('liulangdongwu','注射疫苗')" type="success" @click="chartDialog4()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"rgb(106, 171, 156)","height":"40px"}'></span>
						注射疫苗
					</el-button>
				</el-row>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="jiankangzhuangkuangChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="zhusheyimiaoChart4" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "liulangdongwu/group/dongwuzhonglei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "liulangdongwu/group/xingbie",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog3() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: "liulangdongwu/group/jiankangzhuangkuang",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog4() {
      ...
      this.$nextTick(()=>{
        ...
        var zhusheyimiaoChart4 = echarts.init(document.getElementById("zhusheyimiaoChart4"),'macarons');
        this.$http({
            url: "liulangdongwu/group/zhusheyimiao",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].zhusheyimiao);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].zhusheyimiao
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zhusheyimiaoChart4.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zhusheyimiaoChart4.resize();
                };
            }
        });
      })
	...
	liulangdongwustatusChange(e,row){
		if(row.status==0){
